...
\title{Métodos Quantitativos IV - Lista 1}
...
Calcule a média da população para cada um dos estados brasileiros e informe quais deles possuem maior e menor população média por município.
